...
export default class UserService {
	static create(email: string, derivedPasswordKey: string, encryptionKey: string, preferences: UserPreferences): Promise<User | null> {
		return new Promise((resolve, reject) => {
			console.log('UserService - create user with email:', email, 'and master password derivedPasswordKey:', derivedPasswordKey)
			...
			UserService.getById(userId, true, true)
				.then((user) => {
					if (user) {
						...
						console.log('UserService - getEncryptionKey failed. No user found with id ' + userId)
						resolve(null)
					}
				})
				.catch((err) => {
	...
	static update(userId: string, data: any): Promise<User> {
		return new Promise((resolve, reject) => {
