...
export class AuthController extends AppController {
	...
	/**
	 * Sends user master password hash to client if email matches db record.
	 * @returns void
	 */
	...
	private async generateToken(req: Request, res: Response): Promise<string | null> {
		if (this.user?._id) {
			...
			if (req.cookies.refresh_token) {
				...
				const { verified } = await AuthService.verifyRefreshToken(
					this.user._id,
	...
	public async refreshToken(req: Request, res: Response) {
		...
		const { record, verified } = await AuthService.verifyRefreshToken(
			req.user.userId,
			req.cookies.refresh_token,
		);
