...
import UserService, { User } from "../services/database/UserService";
...
export class UserController extends AppController {
  ...
  public async createUser(req: Request, res: Response) {
    ...
    if (!preferences) return this.clientError(res, "Preferences are required");
  ...
  public async getAll(_: any, res: Response) {
    if (!this.isDev) {
      return this.forbidden(this.res, "this route is only accessible in development environment");
    }
  ...
  public async updateUser(req: Request, res: Response) {
    ...
    if (!user) return this.clientError(res, "Nothing to update");
    const userRecord = await UserService.getById(this.userId);
    if (userRecord && this.userId === userRecord._id) {
      const userUpdated = await UserService.update(this.userId, user);
      if (userUpdated) {
        this.ok(res, { success: userUpdated != null, user: userUpdated });
      }
    }
  }
